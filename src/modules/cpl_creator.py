...
# Instância global do protocolo CPL
cpl_protocol = None

def get_cpl_protocol():
    """Retorna instância global do protocolo CPL"""
    global cpl_protocol
    if cpl_protocol is None:
        cpl_protocol = CPLDevastadorProtocol()
    return cpl_protocol

async def generate_cpl_module(session_id: str, sintese_master: Dict, avatar_data: Dict, contexto_estrategico: Dict, dados_web: Dict) -> Dict[str, Any]:
    """
    Função principal para gerar módulo CPL completo
    Integra com o sistema V70V1 Enhanced
    """
    try:
        logger.info(f"🎬 Iniciando geração de módulo CPL para sessão: {session_id}")
        
        # Extrai dados necessários
        tema = contexto_estrategico.get('tema', sintese_master.get('segmento', 'Negócio Digital'))
        segmento = contexto_estrategico.get('segmento', sintese_master.get('segmento', 'Empreendedorismo'))
        publico_alvo = avatar_data.get('publico_alvo', sintese_master.get('publico_alvo', 'Empreendedores'))
        
        # Obtém protocolo CPL
        protocol = get_cpl_protocol()
        
        # Executa protocolo completo
        resultado_cpl = await protocol.executar_protocolo_completo(
            tema=tema,
            segmento=segmento,
        ...
        # Formata resultado para integração com V70V1
        modulo_cpl = {
            'session_id': session_id,
            'tipo': 'cpl_completo',
            'titulo': f"CPLs Devastadores - {tema}",
            'contexto_estrategico': resultado_cpl['contexto_estrategico'],
            'evento_magnetico': resultado_cpl['evento_magnetico'],
            'cpls_gerados': resultado_cpl['cpls'],
            'dados_busca_utilizados': resultado_cpl.get('dados_busca', {}),
            'timestamp': resultado_cpl['timestamp'],
            'status': 'concluido'
        }
        ...
        logger.error(f"❌ Erro ao gerar módulo CPL: {e}")
        # Retorna módulo de fallback
        return {
            'session_id': session_id,
            'tipo': 'cpl_completo',
            'titulo': f"CPLs Devastadores - Fallback",
            'erro': str(e),
            'status': 'erro',
            'timestamp': datetime.now().isoformat(),
            'fallback_message': 'Módulo CPL não disponível - dados insuficientes'
        }
...
class CPLDevastadorProtocol:
    ...
    def __init__(self):
        # Importa dependências do sistema V70V1
        try:
        ...
        try:
            ...
            self.search_engine = real_search_orchestrator
        except ImportError:
    ...
    async def definir_contexto_busca(self, tema: str, segmento: str, publico_alvo: str) -> ContextoEstrategico:
        ...
        try:
            ...
            contexto_raw = await self.ai_manager.generate_content(
                prompt=prompt,
                provider='qwen',
                max_tokens=2000
            )
    ...
    async def executar_protocolo_completo(self, tema: str, segmento: str, publico_alvo: str, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            resultado_final = {
                ...
                'dados_busca': search_results if isinstance(search_results, dict) else search_results.__dict__ if hasattr(search_results, '__dict__') else {},
                'timestamp': datetime.now().isoformat()
            }
