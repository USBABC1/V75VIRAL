        ...
        def execute_collection():
            try:
                ...
                try:
                    ...
                    viral_results = loop.run_until_complete(
                        viral_integration_service.search_viral_content(
                            query=query,
                            session_id=session_id,
                            max_images=20,
                            min_engagement=50,
    ...
    try:
        ...
        try:
            ...
            for image in response["viral_images"]:
                if image.get('local_image_path'):
                    # Converte caminho absoluto para relativo ao diretório de análises
                    try:
                        relative_path = os.path.relpath(image['local_image_path'], "analyses_data")
                        image['frontend_image_url'] = f"/files/analyses_data/{relative_path}"
                    except:
...
def serve_viral_image(session_id, image_name):
    ...
    try:
        image_path = Path(f"analyses_data/viral_images/{session_id}/{image_name}")
    ...
    if viral_results and not viral_results.get('fallback_used', False):
        ...
        if viral_images:
            ...
            for i, viral_img in enumerate(top_viral, 1):
                ...
                # URL original
                if viral_img.get('original_url'):
                    report += f"**URL Original:** {viral_img.get('original_url')}  \n"
                
                # Imagem local se disponível
                local_path = viral_img.get('local_image_path')
                if local_path and os.path.exists(local_path):
                    relative_path = os.path.relpath(local_path, "analyses_data")
                    report += f"**Imagem Local:** ![Viral {i}]({relative_path})  \n"
